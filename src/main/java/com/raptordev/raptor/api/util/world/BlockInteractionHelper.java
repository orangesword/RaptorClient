package com.raptordev.raptor.api.util.world;
...
public class BlockInteractionHelper
{
    ...
    public static boolean checkForNeighbours(BlockPos blockPos)
    {
        ...
        if (!hasNeighbour(blockPos))
        {
            ...
            for (EnumFacing side : EnumFacing.values())
            {
                ...
                {
                    if (mc.world.getBlockState(blockPos.up()).getBlock() == Blocks.AIR && ModuleManager.getModule(LiquidInteract.class).isEnabled())
                        return true;
                }
            }
            return false;
    ...
    public static ValidResult valid(BlockPos pos)
    {
        ...
        if (mc.world.getBlockState(pos.down()).getBlock() == Blocks.WATER)
            if (ModuleManager.getModule(LiquidInteract.class).isEnabled())
            ...
            for (BlockPos l_Pos : l_Blocks)
            {
                ...
                for (final EnumFacing side : EnumFacing.values())
                {
                    ...
                    if (mc.world.getBlockState(neighbor).getBlock().canCollideCheck(mc.world.getBlockState(neighbor), false)
                            || (l_IsWater && ModuleManager.getModule(LiquidInteract.class).isEnabled()))
                    {
                        return ValidResult.Ok;
                    }
                }
