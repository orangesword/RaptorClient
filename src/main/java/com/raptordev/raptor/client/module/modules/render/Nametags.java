...
public class Nametags extends Module {
    ...
    public ColorSetting borderColor = registerColor("Border Color", new RCColor(255, 0, 0, 255));
    ...
    private RCColor findTextColor(EntityPlayer entityPlayer) {
        ...
        if (SocialManager.isFriend(entityPlayer.getName())) {
            ...
            return new RCColor(239, 1, 71);
        } else if (entityPlayer.isSneaking()) {
            return new RCColor(255, 153, 0);
        }

        return new RCColor(255, 255, 255);
    }
    ...
    private void renderItemName(ItemStack itemStack, int posY) {
        ...
        ColorMain colorMain = ModuleManager.getModule(ColorMain.class);
        FontUtil.drawStringWithShadow(colorMain.customFont.getValue(), itemStack.getDisplayName(), -FontUtil.getStringWidth(colorMain.customFont.getValue(), itemStack.getDisplayName()) / 2, posY, new RCColor(255, 255, 255));
        GlStateManager.popMatrix();
    ...
    private void renderItemDurability(ItemStack itemStack, int posX, int posY) {
        ...
        GlStateManager.scale(0.5, 0.5, 0.5);
        ColorMain colorMain = ModuleManager.getModule(ColorMain.class);
        FontUtil.drawStringWithShadow(colorMain.customFont.getValue(), (int) (damagePercent * 100) + "%", posX * 2, posY, new RCColor((int) (red * 255), (int) (green * 255), 0));
        GlStateManager.popMatrix();
        GlStateManager.disableTexture2D();
    }
    ...
    private void renderEnchants(ItemStack itemStack, int posX, int posY) {
        ...
        for (Enchantment enchantment : EnchantmentHelper.getEnchantments(itemStack).keySet()) {
            ...
            if (showEnchantName.getValue()) {
                ...
                ColorMain colorMain = ModuleManager.getModule(ColorMain.class);
                FontUtil.drawStringWithShadow(colorMain.customFont.getValue(), findStringForEnchants(enchantment, level), posX * 2, posY, new RCColor(255, 255, 255));
            }
            posY += 8;
        }
        ...
        if (itemStack.getItem().equals(Items.GOLDEN_APPLE) && itemStack.hasEffect()) {
            ColorMain colorMain = ModuleManager.getModule(ColorMain.class);
            FontUtil.drawStringWithShadow(colorMain.customFont.getValue(), "God", posX * 2, posY, new RCColor(195, 77, 65));
        }
