...
@Module.Declaration(name = "TargetHUD", category = Category.HUD,toggleMsg = false)
@HUDModule.Declaration(posX = 0, posZ = 70)
public class TargetHUD extends HUDModule {

    IntegerSetting range = registerInteger("Range", 100, 10, 260);
    ColorSetting outline = registerColor("Outline", new RCColor(255, 0, 0, 255));
    ColorSetting background = registerColor("Background", new RCColor(0, 0, 0, 255));
    ...
    private static RCColor getHealthColor(int health) {
        if (health > 36) {
            health = 36;
        }
        if (health < 0) {
        ...
        public void render(Context context) {
            ...
            if (mc.world != null && mc.player.ticksExisted >= 10) {
                ...
                if (entityPlayer != null && entityPlayer.getDistance(mc.player) <= range.getValue()) {
                    // Render background
                    Color bgcolor = new RCColor(background.getValue(), 100);
                    ...
                    RaptorClientGui.renderEntity(entityPlayer, new Point(context.getPos().x + 35, context.getPos().y + 87 - (entityPlayer.isSneaking() ? 10 : 0)), 43);
                    targetPlayer = null;
                    // Render name
                    ...
                    for (ItemStack itemStack : entityPlayer.getArmorInventoryList()) {
                        xPos -= 20;
                        RaptorClientGui.renderItem(itemStack, new Point(xPos, context.getPos().y + 73));
                    }
                }
            }
        }
