...
import com.raptordev.raptor.api.util.player.*;
...
@Module.Declaration(name = "CevBreaker", category = Category.Combat, priority = 999, Description = "haha copy pasted from gs")
public class CevBreaker extends Module {
    ...
    private void breakCrystalPiston(Entity crystal) {
        ...
        if (!breakCrystal.getValue().equalsIgnoreCase("None")) {
            if (rotate.getValue()) {
                // Look at that packet
                SpoofRotationUtil.ROTATION_UTIL.lookAtPacket(vecCrystal.x, vecCrystal.y, vecCrystal.z, mc.player);
