...
public class CombatInfo extends HUDModule {
    ...
    public void onRender() {
        ...
        list.players = mc.world.loadedEntityList.stream()
                ...
                .filter(e -> mc.player.getDistance(e) <= RCCrystalAura.placeRange.getValue())
        ...
        for (EntityPlayer e : entities) {
            ...
            for (BlockPos add : surroundOffset) {
                ...
                if (mc.world.getBlockState(o).getBlock() == Blocks.OBSIDIAN) {
                    if (i == 1 && CrystalUtil.canPlaceCrystal(o.north(1).down(), RCCrystalAura.endCrystalMode.getValue())) {
                        list.lby = true;
                        list.renderLby = true;
                    } else if (i == 2 && CrystalUtil.canPlaceCrystal(o.east(1).down(), RCCrystalAura.endCrystalMode.getValue())) {
                        list.lby = true;
                        list.renderLby = true;
                    } else if (i == 3 && CrystalUtil.canPlaceCrystal(o.south(1).down(), RCCrystalAura.endCrystalMode.getValue())) {
                        list.lby = true;
                        list.renderLby = true;
                    } else if (i == 4 && CrystalUtil.canPlaceCrystal(o.west(1).down(), RCCrystalAura.endCrystalMode.getValue())) {
                        list.lby = true;
                        list.renderLby = true;
                    }
                } else {
        ...
        public Color getItemColor(int index) {
            ...
            if (infoType.getValue().equals("RitomG's")) {
                ...
                if (index == 0) {
                    ...
                    if (players != null) {
                        on = mc.player.getDistance(players) <= RCCrystalAura.breakRange.getValue();
                    }
                } else if (index == 2) {
                    if (players != null) {
                        on = mc.player.getDistance(players) <= RCCrystalAura.placeRange.getValue();
                    }
                } else if (index == 3) {
